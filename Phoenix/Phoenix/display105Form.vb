Public Class display105Form
    ...
    Private isFirstRun_Load As Boolean          '--Tracks if it's the first time the form is loaded--
    ...
    Private Sub dispaly105Form_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Dim i As Integer
        cmb_DataSource.SelectedIndex = 0
        txt_BatchNumber.Text = BATCHNUMBER
        If isCaseOnSQL Then cmb_DataSource.Enabled = True Else cmb_DataSource.Enabled = False
        If isFileSource Then cmb_DataSource.Items(0) = "GUMP File" Else cmb_DataSource.Items(0) = "Manual Entry"
        ...
        If isPreview Then
            ...
            btn_Cancel.Text = "-----"
            btn_GLink.Enabled = False
            btn_GLink.Text = "-----"
            btn_Print.Visible = False
        Else
            If glapiTP8.bool_Visible Then btn_GLink.Text = "Hide GLink" Else btn_GLink.Text = "Show GLink"
            btn_Submit.Enabled = False
        End If

        If Not isPageError Then
            ParentForm_Processing.isRedo_Page1 = False
            ParentForm_Processing.isRedo_Page2 = False
            ParentForm_Processing.isRedo_Page3 = False
            ParentForm_Processing.isRedo_Page4 = False
            ParentForm_Processing.isRedo_Page5 = False
            ParentForm_Processing.isRedo_Page6 = False
            ParentForm_Processing.isRedo_Page9 = False
            ParentForm_Processing.isRedo_Page10 = False
            For i = 0 To ParentForm_Processing.Redo_ChildNum.Length - 1
                ParentForm_Processing.Redo_ChildNum(i) = Nothing
            Next
        Else
            btn_Print.Visible = False
        End If
        ...
        ErrorMessage_Check()
        Highlight_Deletions(Me)
        isFirstRun_Load = False
        tab_Main.Focus()
    End Sub
    Private Sub display105Form_FormClosing(ByVal sender As System.Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles MyBase.FormClosing
        If Not isPageError Then
            set_TotalChange()
            ParentForm_Processing.Redo_totalPages = TotalChangeCount
            ParentForm_Processing.Redo_totalChildren = ChildChangeCount
        End If
        Select Case ClosingCondition
            Case "Submit"
                Transfer105A()
                Transfer105A1()
                If numChildren > 0 Then TransferFormB(ChildNumber)
                If numVRP > 0 Then TransferFormC(VRPNumber)
                ParentForm_Processing.isContinue = True
                ParentForm_Processing.isCancelCase = False
                ParentForm_Processing.isOverride = False
            Case "Override"
                ParentForm_Processing.isContinue = False
                ParentForm_Processing.isCancelCase = False
                ParentForm_Processing.isOverride = True
            Case "Cancel"
                ParentForm_Processing.isContinue = False
                ParentForm_Processing.isCancelCase = True
                ParentForm_Processing.isOverride = False
            Case Else
                If isPreview Then
                    e.Cancel = True
    ...
    Private Sub set_TotalChange()
        If ParentForm_Processing.isRedo_Page1 Then TotalChangeCount += 1
        If ParentForm_Processing.isRedo_Page2 Then TotalChangeCount += 1
        If ParentForm_Processing.isRedo_Page3 Then TotalChangeCount += 1
        If ParentForm_Processing.isRedo_Page4 Then TotalChangeCount += 1
        If ParentForm_Processing.isRedo_Page5 Then TotalChangeCount += 1
        If ParentForm_Processing.isRedo_Page6 Then TotalChangeCount += 1
        If ParentForm_Processing.isRedo_Page9 Then TotalChangeCount += 1
        If ParentForm_Processing.isRedo_Page10 Then TotalChangeCount += 1
    End Sub
    Private Sub set_cmbChild()
        Dim i As Integer
        For i = 0 To numChildren - 1
            cmb_Child.Items.Add(FamisCaseChild(i).QA.GetData & ": " & FamisCaseChild(i).QC.GetData.Replace(" ", "") & " " & FamisCaseChild(i).QB.GetData.Replace(" ", ""))
        Next
    End Sub
    ...
    Private Sub AddEvents(ByVal ctrlparent As Control)
        ...
        For Each ctrl In ctrlparent.Controls
            If TypeOf ctrl Is TextBox Then
                ...
                AddHandler ctrl.TextChanged, AddressOf TrackChanges
            End If
        ...
        Next
    End Sub
    Private Sub TrackChanges(ByVal sender As Object, ByVal e As EventArgs)
        Dim i As Integer = 0
        Dim AddCode As Boolean = True
        If Not isFirstRun_Load And Not isSourceChanging Then
            btn_Submit.Enabled = True
            Select Case DirectCast(sender, TextBox).Name.Substring(4, 2)
                Case "AA", "AB", "AK", "AJ", "AC", "AD", "AE", "AH", "AI", "AL", "AM", "WL", "HC", "AN", "WW", "IA", "IB", "IC", "ID", "IE", "IF", "IF1", "IG", "PC", "PK", "WA", "WB", "WC", "WD", "WE", "WF", "WG", "LA", "LB", "LC", "LD", "LE", "LF", "ND", "AQ", "NQ"
                    ParentForm_Processing.isRedo_Page1 = True
                Case "BA", "BB", "BC", "BD", "BE", "BF", "BG", "BH", "BI", "BS", "BT", "BV", "BJ", "BK", "BL", "BM", "BN", "BO", "BQ", "BR", "BW", "BX", "BZ", "CA", "CB", "CC", "CD", "CE", "CF", "CG", "DA", "DB", "DC", "DD", "DE", "DF", "EA", "EB", "EC", "ED", "EE", "EF", "EG", "EH", "EJ", "EK", "EL", "EM", "EN", "XM", "XN", "XA", "XB", "XC", "XD", "XE", "XF", "XG", "XH", "XI", "XJ", "XK", "XL", "BU", "BV", "B1", "B2", "BY", "BZ", "B3", "B4", "B5", "B6"
                    ParentForm_Processing.isRedo_Page2 = True
                Case "FA", "FB", "FC", "FD", "FE", "FE1", "FE2", "FF", "FG", "FG1", "FG2", "FG3", "FI", "FJ", "FK", "FL", "FM", "FM1", "FM2", "FN", "FP", "FP1", "FP2", "FP3", "GA", "GB", "GC", "GD", "GE", "GF", "GG", "GH", "GI", "GJ", "GK", "GL", "HA", "HB", "HD", "HE", "HF", "HG", "HI", "HJ", "HM", "HN", "HO", "HQ", "HR", "FQ", "FR", "HU", "HV"
                    ParentForm_Processing.isRedo_Page3 = True
                Case "IH", "II", "IJ", "IK", "IL", "IM", "IN", "IO", "JA", "JB", "JC", "JD", "JE", "JF", "JG", "JH", "JI", "JJ", "JL", "JN", "JO", "JP", "JQ", "JR", "JS", "JT", "JU", "JW", "JX", "KA", "KB", "KC", "KD", "KE", "KF", "KG", "KH", "KI", "KJ", "KL", "KN", "KR", "KS", "KU", "KV", "WX", "WY", "VB", "VF", "VH"
                    ParentForm_Processing.isRedo_Page4 = True
                Case "LG", "LH", "LJ", "LK", "LL", "LM", "LN", "LP", "LQ", "LR", "LS", "LT", "MA", "MB", "MC", "MD", "ME", "MF", "MG", "MH", "MI", "MJ", "MK", "ML", "MM", "MN", "MQ", "MR", "NA", "NB", "NC", "NE", "NF", "NG", "NH", "NI", "NJ", "NK", "NL", "NN", "NO", "MS", "MT", "MU", "NQ", "NR", "NS", "NT", "NU", "NV", "NW"
                ...
                Case "QB", "QC", "QD", "QE", "QE1", "QF", "QG", "QH", "QI", "QI1", "QI2", "QJ", "QK", "QL", "QM", "QN", "QO", "RA", "RB", "RC", "RD", "RG", "RH", "RI", "RJ", "RK", "RL", "RM", "RN", "RO", "RP", "RQ", "SA", "SB", "SC", "SD", "SE", "SF", "SG", "SH", "SI", "SJ", "SL", "SN", "SO", "SP", "SQ", "SR", "SS", "TB", "TD", "TE", "TG", "TH", "TI", "TJ", "TK", "TL", "TM", "TO", "TP", "TQ", "TR", "TS", "UA", "UB", "UD", "UE", "UF", "UG", "UH", "UI", "QU", "RT", "RS", "TU"
                    If Not isChildChanging Then
                        For i = 0 To ChildChangeCount - 1
                            ...
                            End If
                        Next
                        If AddCode = True Then
                            ParentForm_Processing.Redo_ChildNum(ChildChangeCount) = ChildNumber
                            ChildChangeCount += 1
                        End If
                    End If
            ...
            End Select
        End If
        If DirectCast(sender, TextBox).Text.Length = DirectCast(sender, TextBox).MaxLength Then
    ...
    Private Sub LoseFocus(ByVal sender As Object, ByVal e As EventArgs)
        ...
        If DirectCast(sender, TextBox).Name = "txt_WC" Or DirectCast(sender, TextBox).Name = "txt_WD" Or DirectCast(sender, TextBox).Name = "txt_WF" Or DirectCast(sender, TextBox).Name = "txt_WG" Then
            DirectCast(sender, TextBox).Text = validateDate(DirectCast(sender, TextBox).Text)
        End If
        switchHighlight(sender)
    End Sub
    Private Sub ToolTip(ByVal sender As Object, ByVal e As EventArgs)
        ...
        Select Case DirectCast(sender, TextBox).Name.Substring(4, 2)
            ...
            Case "EH"
                Note = "HOUSING INDICATOR (CA/MED)" & vbCrLf & "   Enter the type of housing in which each recipient family resides."
            Case "EI"
    ...
    End Sub
    Private Sub ErrorMessage_Check()
        Dim i As Integer
        Dim max As Integer = ErrorMessage.Length - 1
        For i = 0 To max
            If ErrorMessage.Chars(i) = "," Or ErrorMessage.Chars(i) = "." Then Highlight_Textbox(Me, i)
            If i < ErrorMessage.Length - 6 Then
                If ErrorMessage.Substring(i, 7) = "Field: " Then
                    Highlight_Textbox(Me, i + 9)
                End If
            End If
        Next
    End Sub
    Private Sub Highlight_Textbox(ByVal ctrlparent As Control, ByVal i As Integer)
        Dim ctrl As Control
        For Each ctrl In ctrlparent.Controls
            If TypeOf ctrl Is TextBox Then
                If ctrl.Name.Substring(4, 2) = ErrorMessage.Substring(i - 2, 2) Then
                    ctrl.BackColor = Color.Red
                End If
            End If
            If ctrl.HasChildren Then
                Highlight_Textbox(ctrl, i)
            End If
        Next
    End Sub
    Private Sub textReadOnly(ByVal ctrlparent As Control)
        Dim ctrl As Control
        For Each ctrl In ctrlparent.Controls
            If TypeOf ctrl Is TextBox And ctrl.Name <> "txt_ErrorMessage" Then switchReadOnly(ctrl)
            If ctrl.HasChildren Then
                textReadOnly(ctrl)
            End If
        Next
    End Sub
    Private Sub switchReadOnly(ByVal sender As Object)
        If cmb_DataSource.SelectedIndex = 0 Then
            DirectCast(sender, TextBox).ReadOnly = False
        ElseIf cmb_DataSource.SelectedIndex = 1 Then
            DirectCast(sender, TextBox).ReadOnly = True
        End If
    End Sub
    Private Sub Highlight_Deletions(ByVal ctrlparent As Control)
        Dim ctrl As Control
        For Each ctrl In ctrlparent.Controls
            If TypeOf ctrl Is TextBox Then
                If DirectCast(ctrl, TextBox).Text.Length > 0 Then
                    switchHighlight(ctrl)
                End If
            End If
            If ctrl.HasChildren Then
                Highlight_Deletions(ctrl)
            End If
        Next
    End Sub
    Private Sub switchHighlight(ByVal sender As Object)
        If DirectCast(sender, TextBox).Text.Substring(0, 1) = "-" Then
            If DirectCast(sender, TextBox).BackColor <> Color.Red Then DirectCast(sender, TextBox).BackColor = System.Drawing.Color.FromArgb(CType(CType(250, Byte), Integer), CType(CType(252, Byte), Integer), CType(CType(110, Byte), Integer))
        Else
            If DirectCast(sender, TextBox).BackColor <> Color.Red Then DirectCast(sender, TextBox).BackColor = Color.White
        End If
    End Sub
    Private Function validateDate(ByVal tempDate As String) As String
        If isValidDate(tempDate) Then
            Return tempDate
        Else
            Return " ".PadLeft(tempDate.Length, " ")
        End If
    End Function
    ...
    Private Sub Fill105A()
        ...
        txt_AQ.Text = FAMISCaseInformation.AQ.GetData
        ...
        txt_B1.Text = FAMISIndividualsInformation.B1.GetData
        txt_B2.Text = FAMISIndividualsInformation.B2.GetData
        txt_B3.Text = FAMISIndividualsInformation.B3.GetData
        txt_B4.Text = FAMISIndividualsInformation.B4.GetData
        txt_B5.Text = FAMISIndividualsInformation.B5.GetData
        txt_B6.Text = FAMISIndividualsInformation.B6.GetData
        txt_FQ.Text = FAMISIndividualsInformation.FQ.GetData
        txt_FR.Text = FAMISIndividualsInformation.FR.GetData
        txt_FH.Text = FAMISIndividualsInformation.FH.GetData
        txt_FO.Text = FAMISIndividualsInformation.FO.GetData
        ...
        txt_HU.Text = FAMISMedicaidInformation.HU.GetData
        txt_HV.Text = FAMISMedicaidInformation.HV.GetData
        txt_XO.Text = FAMISMedicaidInformation.XO.GetData
        txt_XP.Text = FAMISMedicaidInformation.XP.GetData
        txt_XQ.Text = FAMISMedicaidInformation.XQ.GetData
        txt_XR.Text = FAMISMedicaidInformation.XR.GetData
        txt_XS.Text = FAMISMedicaidInformation.XS.GetData
        txt_XT.Text = FAMISMedicaidInformation.XT.GetData
        txt_XU.Text = FAMISMedicaidInformation.XU.GetData
        txt_XV.Text = FAMISMedicaidInformation.XV.GetData
    ...
    Private Sub Fill105A1()
        ...
        txt_MS.Text = FAMISFoodStampInformation.MS.GetData
        txt_MT.Text = FAMISFoodStampInformation.MT.GetData
        txt_MU.Text = FAMISFoodStampInformation.MU.GetData
        txt_NQ.Text = FAMISFoodStampInformation.NQ.GetData
        txt_NR.Text = FAMISFoodStampInformation.NR.GetData
        txt_NS.Text = FAMISFoodStampInformation.NS.GetData
        txt_NT.Text = FAMISFoodStampInformation.NT.GetData
        txt_NU.Text = FAMISFoodStampInformation.NU.GetData
        txt_NV.Text = FAMISFoodStampInformation.NV.GetData
        txt_NW.Text = FAMISFoodStampInformation.NW.GetData
        txt_OP.Text = FAMISFoodStampInformation.OP.GetData
        txt_OR.Text = FAMISFoodStampInformation.OR.GetData
        txt_VB.Text = FAMISFoodStampInformation.VB.GetData
        txt_VF.Text = FAMISFoodStampInformation.VF.GetData
        txt_VH.Text = FAMISFoodStampInformation.VH.GetData
        txt_OQ1.Text = FAMISFoodStampInformation.OQ1.GetData
        txt_oq2.text = FAMISFoodStampInformation.OQ2.GetData
    ...
    Private Sub FillFormB(ByVal PersonNumber As Integer)
        ...
        txt_QU.Text = FAMISCaseChild(PersonNumber).QU.GetData
        txt_RS.Text = FAMISCaseChild(PersonNumber).RS.GetData
        txt_RT.Text = FAMISCaseChild(PersonNumber).RT.GetData
        txt_TT.Text = FAMISCaseChild(PersonNumber).TT.GetData
    ...
    Private Sub FillFormC(ByVal PaymentNumber As Integer)
        ...
        txt_VC.Text = FAMISVRPInformation(0).VC.GetData
        txt_OL.Text = FAMISFoodStampInformation.OL.GetData
        txt_PO.Text = FAMISIandAInformation.PO.GetData
    ...
    Private Sub Transfer105A()
        ...
        FAMISCaseInformation.AQ.SetData(txt_AQ.Text)
        ...
        FAMISIndividualsInformation.FD2.SetData(txt_FD2.Text)
        ...
        'FAMISIndividualsInformation.FH.SetData(txt_FH.Text)   --Taken out 1/15/2008 Computer Generated and Readonly--
        ...
        FAMISIndividualsInformation.FL2.SetData(txt_FL2.Text)
        ...
        'FAMISIndividualsInformation.FO.SetData(txt_FO.Text)   --Taken out 1/15/2008 Computer Generated and Readonly--
        ...
        FAMISIndividualsInformation.B1.SetData(txt_B1.Text)
        FAMISIndividualsInformation.B2.SetData(txt_B2.Text)
        FAMISIndividualsInformation.B3.SetData(txt_B3.Text)
        FAMISIndividualsInformation.B4.SetData(txt_B4.Text)
        FAMISIndividualsInformation.B5.SetData(txt_B5.Text)
        FAMISIndividualsInformation.B6.SetData(txt_B6.Text)
        FAMISIndividualsInformation.FQ.SetData(txt_FQ.Text)
        FAMISIndividualsInformation.FR.SetData(txt_FR.Text)
        ...
        FAMISMedicaidInformation.HU.SetData(txt_HU.Text)
        FAMISMedicaidInformation.HV.SetData(txt_HV.Text)
        FAMISMedicaidInformation.XO.SetData(txt_XO.Text)
        FAMISMedicaidInformation.XP.SetData(txt_XP.Text)
        FAMISMedicaidInformation.XQ.SetData(txt_XQ.Text)
        FAMISMedicaidInformation.XR.SetData(txt_XR.Text)
        FAMISMedicaidInformation.XS.SetData(txt_XS.Text)
        FAMISMedicaidInformation.XT.SetData(txt_XT.Text)
        FAMISMedicaidInformation.XU.SetData(txt_XU.Text)
        FAMISMedicaidInformation.XV.SetData(txt_XV.Text)
    ...
    Private Sub Transfer105A1()
        ...
        FAMISFoodStampInformation.MS.SetData(txt_MS.Text)
        FAMISFoodStampInformation.MT.SetData(txt_MT.Text)
        FAMISFoodStampInformation.MU.SetData(txt_MU.Text)
        FAMISFoodStampInformation.NQ.SetData(txt_NQ.Text)
        FAMISFoodStampInformation.NR.SetData(txt_NR.Text)
        FAMISFoodStampInformation.NS.SetData(txt_NS.Text)
        FAMISFoodStampInformation.NT.SetData(txt_NT.Text)
        FAMISFoodStampInformation.NU.SetData(txt_NU.Text)
        FAMISFoodStampInformation.NV.SetData(txt_NV.Text)
        FAMISFoodStampInformation.NW.SetData(txt_NW.Text)
        FAMISFoodStampInformation.OP.SetData(txt_OP.Text)
        FAMISFoodStampInformation.OR.SetData(txt_OR.Text)
        FAMISFoodStampInformation.VB.SetData(txt_vb.text)
        FAMISFoodStampInformation.VF.SetData(txt_VF.Text)
        FAMISFoodStampInformation.VH.SetData(txt_VH.Text)
        FAMISFoodStampInformation.OQ1.SetData(txt_OQ1.Text)
        FAMISFoodStampInformation.OQ2.SetData(TXT_OQ2.TEXT)
    ...
    Private Sub TransferFormB(ByVal PersonNumber As Integer)
        ...
        FAMISCaseChild(PersonNumber).RH2.SetData(txt_RH2.Text)
        ...
        FAMISCaseChild(PersonNumber).QU.SetData(txt_QU.Text)
        FAMISCaseChild(PersonNumber).RS.SetData(txt_RS.Text)
        FAMISCaseChild(PersonNumber).RT.SetData(txt_RT.Text)
        FAMISCaseChild(PersonNumber).TT.SetData(txt_TT.Text)
    ...
    Private Sub TransferFormC(ByVal PaymentNumber As Integer)
        ...
        FAMISVRPInformation(0).VC.SetData(txt_VC.Text)
        FAMISFoodStampInformation.OL.SetData(txt_OL.Text)
        FAMISIandAInformation.PO.SetData(txt_PO.Text)
    ...
    Private Sub cmb_Child_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmb_Child.SelectedIndexChanged
        ...
        If isFirstRun_Child = False Then
            ...
            Else
                ChildNumber = cmb_Child.SelectedIndex
                FAMISCaseChildTableAdapter.Fill(PhoenixDataSet.FAMISCaseChild, CASENUMBER, FamisCaseChild(ChildNumber).QA.GetData)
            End If
        Else
            isFirstRun_Child = False
        End If
        isChildChanging = False
        Highlight_Deletions(tab_105B)
        cmb_Child.Focus()
    End Sub
    Private Sub cmb_VRP_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmb_VRP.SelectedIndexChanged
        If isFirstRun_VRP = False Then
            ...
            Else
                VRPNumber = cmb_VRP.SelectedIndex
                FAMISVRPInformationTableAdapter.Fill(PhoenixDataSet.FAMISVRPInformation, CASENUMBER, (VRPNumber + 1).ToString.PadLeft(2, "0"))
            End If
        Else
            isFirstRun_VRP = False
        End If
        Highlight_Deletions(tab_VRP)
    End Sub
    Private Sub cmb_DataSource_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmb_DataSource.SelectedIndexChanged
        isSourceChanging = True
        Me.Cursor = Cursors.WaitCursor
        If cmb_DataSource.SelectedIndex = 0 Then
            textReadOnly(Me)
            PhoenixDataSet.Clear()
            Fill105A()
            Fill105A1()
            If numChildren <> 0 Then FillFormB(ChildNumber)
            If numVRP <> 0 Then FillFormC(VRPNumber)
            btn_GLink.Enabled = True
    ...
    Private Sub btn_Submit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_Submit.Click
        ClosingCondition = "Submit"
        Me.Close()
    End Sub
    Private Sub btn_Override_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_Override.Click
        ClosingCondition = "Override"
        Me.Close()
    End Sub
    Private Sub btn_Close_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_Cancel.Click
        ClosingCondition = "Cancel"
        Me.Close()
    End Sub
    Private Sub btn_GLink_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_GLink.Click
        If glapiTP8.bool_Visible Then
            glapiTP8.SetVisible(False)
            btn_GLink.Text = "Show GLink"
        Else
            glapiTP8.SetVisible(True)
            btn_GLink.Text = "Hide GLink"
        End If
    End Sub
    Private Sub btn_Print_LinkClicked(ByVal sender As System.Object, ByVal e As System.Windows.Forms.LinkLabelLinkClickedEventArgs) Handles btn_Print.LinkClicked
        glapiTP8.PrintScreen()
    End Sub

    Private Sub DatabaseView()
        ...
        FAMISAppendedInformationTableAdapter.Fill(PhoenixDataSet.FAMISAppendedInformation, CASENUMBER)
        If numChildren <> 0 Then FAMISCaseChildTableAdapter.Fill(PhoenixDataSet.FAMISCaseChild, CASENUMBER, FAMISCaseChild(ChildNumber).QA.GetData)
        If numVRP <> 0 Then FAMISVRPInformationTableAdapter.Fill(PhoenixDataSet.FAMISVRPInformation, CASENUMBER, (VRPNumber + 1).ToString.PadLeft(2, "0"))
    End Sub

    Private Sub display105Form_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
        If e.KeyCode = Keys.Right Or e.KeyCode = Keys.Up Then
            tab_Main.SelectedIndex = tab_Main.SelectedIndex + 1
