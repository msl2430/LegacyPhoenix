...
Module MedicaidModule
    ...
    Public ParentControlScreen As MainScreen
    ...
    Public Sub ProcessMedicaid()
        ...
        '------------------------
        For i = 0 To MaxMedi - 1
            If ParentControlScreen.bgw_ScanDirectory.CancellationPending Then Exit For
            ...
            Try
                ...
                Else
                    ...
                    ParentControlScreen.bgw_ScanDirectory.ReportProgress(7)
                    Thread.Sleep(1578)
                End If
            ...
            If Not MedicaidFileList(i).isDone And isOperatorOK Then
                ...
                ParentControlScreen.bgw_ScanDirectory.ReportProgress(2)
                GLinkStartMedi()
                                        ...
                                        If isOPTError() And (ErrorMessage1Medi.Substring(0, 10) = " HIGHLIGHT" Or ErrorMessage1Medi.Substring(0, 10) = "HIGHLIGHT ") Then
                                            ...
                                            For j = i + 1 To MaxMedi - 1
                                                If MedicaidFileList(j).CaseNumber = MediCaseNumber And MedicaidFileList(j).PersonNumber = MediPersonNumber Then
                                                    ...
                                                    If File.Exists(DropDirectory & "\" & MedicaidFileList(j).FileName) Then File.Delete(DropDirectory & "\" & MedicaidFileList(j).FileName)
                                                    File.Move(MedicaidFileList(j).FilePath, DropDirectory & "\" & MedicaidFileList(j).FileName)
                                                    ParentControlScreen.bgw_ScanDirectory.ReportProgress(5)
                ...
                Else
                    '--GLink error--
                    ErrorMessage2Medi = ""
                    ParentControlScreen.bgw_ScanDirectory.ReportProgress(8)
                    Exit For
                End If
                Try
    ...
    Private Sub AddFamily(ByVal FileIndex As Integer)
        ...
        Dim HoldDirectory As New DirectoryInfo(FileDirectory & "\FamilyHolds\")
        Dim FileList() As FileInfo
        Dim tempFile As MedicaidFile
        FamilyList.Add(MedicaidFileList(FileIndex))
        ParentControlScreen.bgw_ScanDirectory.ReportProgress(20)
        For i = 0 To MaxMedi - 1
        ...
        If FileList.Length > 0 Then
            For i = 0 To FileList.Length - 1
                tempFile = New MedicaidFile(FileDirectory & "\FamilyHolds\" & FileList(i).Name)
                If tempFile.CaseNumber = MedicaidFileList(FileIndex).CaseNumber Then
                    If File.Exists(FileDirectory & "\" & FileList(i).Name) Then
                        File.Delete(FileDirectory & "\FamilyHolds\" & FileList(i).Name)
                    Else
                        File.Move(FileDirectory & "\FamilyHolds\" & FileList(i).Name, FileDirectory & "\" & FileList(i).Name)
                        MedicaidFileList(MaxMedi) = New MedicaidFile(FileDirectory & "\" & FileList(i).Name)
        ...
        For i = 0 To FamilyList.Count - 1
            If Not FamilyList(i).isDone Then
                ...
                If Not isFirstRun Then ParentControlScreen.bgw_ScanDirectory.ReportProgress(2)
                If Not isErrorMedi Then
                    Read_TextFile()
                    Thread.Sleep(750)
                    Select Case OPTChoice
    ...
    Private Sub StoreErrorOpt(ByVal index As Integer)
        ParentControlScreen.bgw_ScanDirectory.ReportProgress(3)
        If OPTChoice = "66" And (ErrorMessage1Medi = " CANNOT ADD REDETRMIN WITHOUT PERS " Or ErrorMessage2Medi = " CANNOT ADD REDETRMIN WITHOUT PERS " Or ErrorMessage1Medi = " PERSON NUMBER NOT FOUND           ") Then
            If DateDiff(DateInterval.Day, File.GetLastWriteTime(MediFile), Date.Today, FirstDayOfWeek.Sunday, FirstWeekOfYear.Jan1) < 4 Then
                If File.Exists(FileDirectory & "\Holds\" & MedicaidFileList(index).FileName) Then File.Delete(FileDirectory & "\Holds\" & MedicaidFileList(index).FileName)
                File.Move(MediFile, FileDirectory & "\Holds\" & MedicaidFileList(index).FileName)
                ErrorMessage1Medi = "Person not on server. Holding case."
                ErrorMessage2Medi = ""
                ParentControlScreen.bgw_ScanDirectory.ReportProgress(6)
            Else
                If File.Exists(DropDirectory & "\" & MedicaidFileList(index).FileName) Then File.Delete(DropDirectory & "\" & MedicaidFileList(index).FileName)
                File.Move(MediFile, DropDirectory & "\" & MedicaidFileList(index).FileName)
                ParentControlScreen.bgw_ScanDirectory.ReportProgress(5)
            End If
    ...
    Private Sub StoreOpt(ByVal index As Integer)
        ...
        Thread.Sleep(500)
        StoreResult("SUCCESS")
        ParentControlScreen.bgw_ScanDirectory.ReportProgress(4)
    End Sub
    ...
    Private Sub StoreResult(ByVal Result As String)
        Dim Worker As String = MediFile.Substring(MediFile.LastIndexOf("\") + 1).Substring(20, 6) 'MediFile.Substring(FileDirectory.Length + 1).Substring(20, 6)
        Dim SQLConn As New SqlConnection(ConnectionString)
        Dim SQLComm As New SqlCommand
        ...
        Try
            ...
            SQLComm.ExecuteNonQuery()
            SQLComm.CommandText = "INSERT INTO TransactionLog VALUES ('" & MediCaseNumber & "', '" & MediFile.Substring(FileDirectory.Length + 1) & "', '" & DateEntered & "', '" & Worker & "', '" & OPTChoice & "', '" & MediPersonNumber & "',  '" & Result & "', '" & ErrorMessage1Medi & ErrorMessage2Medi & "', 'SERVER', '" & isReported & "')"
            SQLComm.ExecuteNonQuery()                                                               '.Substring(FileDirectory.Length + 1)
        Catch ex As Exception
    ...
    Private Sub GLinkStartMedi()
        ...
        While isLoop
            ...
            If glapiMedicaid.isConnect() Then
                ...
                End While
                Thread.Sleep(500)
                glapiMedicaid.SubmitField(16, MediUser)
                glapiMedicaid.SubmitField(19, MediPassword)
                glapiMedicaid.TransmitPage()
